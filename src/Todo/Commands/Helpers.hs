...
getPendingTodo = do
  path <- get >>= (return . T.pack . todoTxtPath)
  tasks <- readFileSafe path >>=
              (\lines -> either (throwError . EParseError) return $ parseLines (T.unpack path) (T.unpack lines))
...
getArchivedTodo = do
  archivePath <- get >>= (return . archiveTxtPath)
  todoPath <- get >>= (return . todoTxtPath)
  let path = fromJust $ mplus archivePath (Just $ replaceFileName todoPath defaultArchiveName)
  tasks <- readFileSafe (T.pack path) >>=
              (\lines -> either (throwError . EParseError) return $ parseLines path (T.unpack lines))
...
writeReport pending archived = do
  reportPath <- get >>= (return . reportTxtPath)
  todoPath <- get >>= (return . todoTxtPath)
...
queryAction' tasks str =
    mapM_ (liftIO . putStrLn . show) tasks >>
...
splitIndexTasks indexes tasks = do
  let matches = filter ((flip elem indexes) . fst) tasks
  let nonMatches = filter ((flip notElem indexes) . fst) tasks
  if (length matches == length indexes)
  then return (matches, nonMatches)
  else throwError $ EInvalidIndexes $ filter (flip notElem (map fst matches)) indexes
